...
In this assignment we will download data at a NUTS 3 level.
...
The UK was not included in the datasets obtained from Eurostat, since the country is no longer a part of the EU.
We got sub-regional GDP and population data for the UK through Office for National Statistics.\
UK use another classification system than EU, instead of NUTS they use ITL regions.
These two classification systems match and we will further in the assignment refer to the regions in UK as NUTS.
...
```{r}
# Merging gdp_dataset with population_dataset 
merged_data <- gdp_data |>
  left_join(population_data, join_by(year,geo))
```

```{r}
# Merging gdp_UK with population_UK 
merged_UK <- gdp_UK |>
...
```{r}
# We only want data from year 2000 - 2020 for the UK dataset
main_data <- main_data %>%
...
Now that our datasets are joined, we can create a new variable for GDP per capita by dividing "GDP" column by the "pop" column.
...
We will take a closer look at the components of GDP per capita.
According to the metadata for the population dataset downloaded form Eurostat, the population in the dataset is given at the end of the calendar year and includes statistics on live births and deaths during the year.
At a NUTS 3 level population is broken down by sex and three age groups (0-14, 15-64 and 65+).
The metadata for the gdp dataset from Eurostat ...++ metadata for the UK datasets gdp and population... ++
