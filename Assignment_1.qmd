...
For the estimation we will use a subset including observations on nuts 2 regions from the year 2010.
We will estimate three models.
The first model include data from all our countries, the second model focuses only on EU countries, while the third model is all about the UK, our largest country with the most nuts 2 regions.
...
The R-squared values indicate the proportion of variation in regional inequality (Gini coefficient) that can be explained by regional develompent (GDP per capita) in each model.
...
While all three models show statistically significant coefficients for "GDP_per_capita," the p-value for the constant term in Model 3 is the highest (p \< 0.001), and the only costant term with stars.
...
1.  **Linearity:** *The population relationship is linear in parameters with an additive error term.*
    ...
    The regression model $\hat{GINIW_{2010}} = \hat{\beta}_1 + \hat{\beta}_2 y_{2010}$have a linear structure between the dependent variable and the independent variable.\

2.  *SLR:* **Sample Variation:** *There is variation in X vs.*\
...
3.  **Random Sampling:** *We have a random sample from the population of interest.*
    ...
    The data used to estimate the regression models is obtained from Eurostat and the Office for National Statistics.
...
4.  **Exogeneity:** *The X variable is exogenous (i.e.,* E(*u*\|X) = 0)\

5.  **Homoskedasticity**: *The error term has the same variance for each value of the independent variable (i.e.,* Var(*u*\|X) = $\sigma^2$)\

6.  **Normality:** *The population error term is normally distributed with mean zero and variance* $\sigma^2$ (*i.e.*, *u* \~ *N*(0,$\sigma^2$ ))
...
plot_all <- subset_nuts2_2010 %>%
  ggplot(aes(x = I(gdp_per_capita_n2 * 100), y = gini_n2)) +
  geom_point() +
...
We hypothesize ....
to may influence regional inequality in Turkey.
We believe these three can have an impact on regional inequality because... JUSTIFY!
We retrieved data for these potential determinants from Eurostat.

Unemployment per thousand persons:

```{r}
# Data acquistion
unemployment_data <- read_csv("lfst_r_lfu3pers__custom_8264488_linear.csv") |>
  select(id_nuts2 = geo, year = TIME_PERIOD, unemployment = OBS_VALUE)
```

infrastructure: road,rail and inland waterways - km per thousand square
