...
```{r}
# Create the scatterplot
scatter_plot1 <- ggplot(subset_nuts2data, aes(x = pop, y = gini_n2, color = id_nuts2)) +
  geom_point() +
...
Without outlier from Turkie and the UK\
...
# Add the regression line with y-axis limits
scatter_plot <- scatter_plot +
  geom_smooth(
    method = "lm",
    se = FALSE,
    linetype = "dashed",
    color = "black",
    aes(ymax = 0.2)  # Set the y-axis limit for the regression line
  )
