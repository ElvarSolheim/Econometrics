...
merged_data <- gdp_data %>%
  left_join(population_data, by = "geo")
```

```{r}
# Merging the datasets together UK
merged_data_UK <- gdp_UK %>%
  left_join(population_UK, by = "geo")
```

```{r}
# Merging the UK with the primary dataset
combined_merged_data <- rbind(merged_data, merged_data_UK)
```
