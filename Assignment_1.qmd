...
The subset analysis includes four subsets, each corresponding to one of our countries.
...
# Regression table
huxreg(
  list("Model 1" = lm4, "Model 2" = lm5, "Model 3" = lm6, "Model 4" = lm7),
  statistics = c(N = "nobs", R2 = "r.squared"),
  coefs = c(GDP_per_capita = "I(gdp_per_capita_n2 * 100)", Constant = "(Intercept)")
  )
```
...
Examining the "GDP per capita" coefficients for the EU contries, we observe that for the Czech Republic, the estimated decrease in regional inequality is 0.008 for a one-unit increase in GDP per capita.
...
The coefficient for GDP per capita is negative for all these countries, but not statistically significant.

The UK stands out with a significant positive relationship between regional inequality and regional development compared to the EU countries.
The coefficient for GDP per capita for the UK is 0.032.
This implies that as GDP per capita increases, the Gini coefficient also increases, indicating a rise in regional inequality.
The coefficient for GDP per capita is highly significant at a 1% significance level.
...
The UK follows with an R-squared value of 0.602, which also is a substantial amount, suggesting a good fit of the model.
The Czech Republic has a R-squared values of 0.223, indicating that the model explains 22.3% of the variance in the Gini coefficient.
...
The second part involves exploring alternative functional forms, where we decided to focus on logaritmic and quadratic forms.
An estimation and visualization of the models will be given.
...
Since the number of observations for each of the EU-countries are small, we will stick to the model with all the EU countries together.
We will explore functional forms for both the EU countries and the UK.

The logarithmic form often appears in models where diminishing returns or asymptotic behavior is expected.
A logarithmic form for the EU countries.
The quadratic form is used to model situations where there's a curvature relationship between variables.
The UK data displays a inverted U-shaped pattern, so a quadratic form might be suitable.
\

### Estimation and Visualization

We will estimate one logaritmic model for the EU countries and one quadratic model for the UK.
